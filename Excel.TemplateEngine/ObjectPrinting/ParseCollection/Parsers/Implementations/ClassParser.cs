...
internal class ClassParser : IClassParser
{
    ...
    public TModel Parse<TModel>(ITableParser tableParser, RenderingTemplate template, Action<string, string> addFieldMapping)
        where TModel : new()
    {
        var model = new TModel();
        Parse(tableParser, template, addFieldMapping, ref model);
        return model;
    }

    public void Parse<TModel>(ITableParser tableParser, RenderingTemplate template, Action<string, string> addFieldMapping, ref TModel model)
        where TModel : new()
    {
    ...
    private Dictionary<ExcelTemplatePath, int> GetEnumerablesLengths<TModel>(ITableParser tableParser, RenderingTemplate template)
    {
    ...
    private void ParseCellularValue(ITableParser tableParser,
                                    Action<string, string> addFieldMapping,
                                    object model,
                                    ExcelTemplatePath path,
                                    Dictionary<ExcelTemplatePath, int> enumerablesLengths)
    {
    ...
    private void ParseSingleValue(ITableParser tableParser,
                                  Action<string, string> addFieldMapping,
                                  object model,
                                  Action<object, object> leafSetter,
                                  ExcelTemplatePath childModelPath,
                                  Type childModelType)
    {
    ...
    private void ParseEnumerableValue(ITableParser tableParser,
                                      Action<string, string> addFieldMapping,
                                      object model,
                                      ExcelTemplatePath path,
                                      Action<object, object> leafSetter,
    ...
    private static (string formControlType, string formControlName) GetFormControlDescription(ICell cell)
    {
