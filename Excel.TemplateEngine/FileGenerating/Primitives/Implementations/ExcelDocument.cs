...
{
    public ExcelDocument(byte[] template, ILog logger)
    {
        this.logger = logger;
        ...
        documentStyle = new ExcelDocumentStyle(
            spreadsheetDocument.GetOrCreateSpreadsheetStyles(logger),
            spreadsheetDocument.WorkbookPart?.ThemePart?.Theme ?? theme,
            this.logger);
    ...
    {
        ThrowIfSpreadsheetDisposed();
        return spreadsheetDocument.WorkbookPart?.Workbook.GetFirstChild<Sheets>()?.Elements<Sheet>().Count() ?? 0;
    }
    ...
    public void CopyDefinedNamesFrom(IExcelDocument sourceExcelDocument)
    {
        ...
        {
            spreadsheetDocument.WorkbookPart!.Workbook.DefinedNames!.AppendChild(definedName.CloneNode(true));
        }
    }
    ...
    public void RenameWorksheet(int index, string name, bool updateDefinedNames = false)
    {
        ...
        var sheetBeforeRenaming = (Sheet)spreadsheetDocument.WorkbookPart!.Workbook.Sheets!.Elements<Sheet>().ElementAt(index).CloneNode(true);
    ...
    private static void AssertWorksheetNameValid(string worksheetName)
    {
    ...
    private void AssertWorksheetExists(int index)
    {
        if (spreadsheetDocument.WorkbookPart?.Workbook.Sheets?.Elements<Sheet>().ElementAt(index) == null)
            throw new InvalidOperationException($"Worksheet {index} doesn't exist");
    }
