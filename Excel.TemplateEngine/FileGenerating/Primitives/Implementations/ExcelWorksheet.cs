...
internal class ExcelWorksheet : IExcelWorksheet
{
    ...
    {
        if (!rowsCache.TryGetValue((uint)position.RowIndex, out var row))
            return null;
        var cell = row
                   .Elements<Cell>()
                   .FirstOrDefault(c => new ExcelCellIndex(c.CellReference).ColumnIndex == position.ColumnIndex);
        return cell == null ? null : new ExcelCell(cell, documentStyle, excelSharedStrings);
    }
